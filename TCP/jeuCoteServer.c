...
int recoitUnMessage(char* buffer)
{
	...
	case 1:
		...
		recoitUnNouveauJoueur(pseudoDuNouveauJoueur); 
		free(pseudoDuNouveauJoueur);
	case 2:
		recoitAnnonce((int) buffer[1] -1, (int) buffer[2] -1); 
	case 4:
		recoitCarteJoueur((int) buffer[1] -1, (int) buffer[2] -2); 
	}
...
void recoitUnNouveauJoueur(char pseudoDuNouveauJoueur[TAILLE_MAX_PSEUDO])
{
	if (partie->laPartieEstLancee == 0)
	{
		...
		if 		(partie->joueurs[0] == NULL) partie->joueurs[0] = nouveauJoueur;
		else if (partie->joueurs[1] == NULL) partie->joueurs[1] = nouveauJoueur;
		else if (partie->joueurs[2] == NULL) partie->joueurs[2] = nouveauJoueur;
		...
		for (int indiceJoueur = 0; indiceJoueur < 3; ++indiceJoueur)
		{
			...
			messageAvecLesPseudos[indiceCharAEcraser++] = 1; // séparateur de pseudos			
		}

		messageAvecLesPseudos[indiceCharAEcraser-1] = '\0';

		for (int indiceJoueur = 0; indiceJoueur < 3; ++indiceJoueur)
			write_client(indiceJoueur, messageAvecLesPseudos);

		free(messageAvecLesPseudos);

		if (partie->joueurs[2] != NULL) lancerUnTourDeJeu();
	}
}

//Gère le cas où on reçoit des annonces
void recoitAnnonce(int indiceJoueur, int nombreDePlis)
{
	if (indiceJoueur == partie->indiceJoueurQuiALaParole)
	{
		...
		if (partie->joueurs[partie->indiceJoueurQuiALaParole]->nombrePlisAnnoncees == -1)
			envoyerMessageTourDAnnonces(0);

		else
		{
			envoyerMessageTourDAnnonces(1);

			printf("TOUR DE CARTES\n");

			if (partie->nombreDePlis != 0)
				envoyerCartesJouees(0);
...
{	
	...
	for (int indiceJoueur = 0; indiceJoueur < 3; ++indiceJoueur)
	{
		...
		write_client(indiceJoueur, messageCartesEnMain);
...
{	
	...
	for (int indiceJoueur = 0; indiceJoueur < 3; ++indiceJoueur)
		write_client(indiceJoueur, messageTourDAnnonces);

	free(messageTourDAnnonces);
}

//Envoie les cartes jouées à tous les joueurs
void envoyerCartesJouees(int signal)
{
	sleep(1); 
	...
	for (int indiceJoueur = 0; indiceJoueur < 3; ++indiceJoueur)
		messageCartesJouees[indiceJoueur + 1] = partie->joueurs[indiceJoueur]->carteJouee + 2; 
	...
	for (int indiceJoueur = 0; indiceJoueur < 3; ++indiceJoueur)
		write_client(indiceJoueur, messageCartesJouees);
...
void envoyerLesScores()
{
	...
	for (int indiceJoueur = 0; indiceJoueur < 3; ++indiceJoueur)
		write_client(indiceJoueur, messageScores);
	free(messageScores);
}
